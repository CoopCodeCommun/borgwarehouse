...
##### INSTRUCTION
#
# Configure une sauvegarde borg vers un serveur borgwarehouse (BWH).
# Lance par :  make init
#
...
BACKUP_TYPE="$(valeur_env BACKUP_TYPE)"
BORG_PREFIX="$(valeur_env BORG_PREFIX)"
BORG_REPO="$(valeur_env BORG_REPO)"
BORG_PASSPHRASE="$(valeur_env BORG_PASSPHRASE)"

case "$BACKUP_TYPE" in
  folder|postgres|sqlite) : ;;
  *) erreur "BACKUP_TYPE doit valoir folder, postgres ou sqlite dans le .env." ;;
esac
SCRIPT_BACKUP="$SCRIPT_DIR/backup_${BACKUP_TYPE}_example.sh"
...
echo
dire "Configuration d'une sauvegarde de type : $BACKUP_TYPE"
echo

if [ "$REPRISE" = 0 ]; then
  BORG_PREFIX="$(demander "Prefixe des archives (nomme aussi la cle SSH)" "$(hostname -s)-$BACKUP_TYPE")"
      ...
      --data-binary @- <<EOF || erreur "l'appel a l'API BWH a echoue (token invalide ? URL incorrecte ?). Tu peux relancer make init : rien n'est perdu."
{
  ...
  "storageSize": $QUOTA,
  "comment": "$BACKUP_TYPE — cree par make init",
  "alert": $ALERTE,
  "lanCommand": false,
